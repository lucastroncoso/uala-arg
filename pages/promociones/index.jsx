...
export default function Promociones(props) {
  ...
  const [allPromotions, setAllPromotions] = useState(
    props.response_promos.argentinaPromotionCollection.items,
  ...
  const [categories, setCategories] = useState(
    props.response_others.argentinaPromotionCategoryCollection.items,
  ); // Lista de categorias - filtro
  const [locations, setLocations] = useState(
    props.response_others.argentinaPromotionLocationCollection.items,
  ); // Lista de ubicaciones - filtro
  ...
  const [banners, setBanners] = useState(
    props.response_others.promotionBannerArgentinaCollection.items,
  ); // Banners
    ...
    if (!!featuredPromotions) {
      const otherPromotions = allPromotions.filter((promo) => {
        ...
        if (featuredPromotions.length === 2) {
          return (
            promo.slug !== featuredPromotions[0].slug && promo.slug !== featuredPromotions[1].slug
          );
        }
      });
    ...
    if (!!promotions && !!featuredPromotions) {
      ...
      if (selectedCategory.slug !== '') {
        if (selectedCategory.slug !== 'any-category') {
          ...
          filteredFeaturedPromotions = filteredFeaturedPromotions.filter((promotion) => {
            if (
              promotion.categoriesCollection.items.find(
                (element) => element.slug === selectedCategory.slug,
              ) !== undefined
            ) {
              return true;
            } else {
      ...
      if (selectedLocation.slug !== '') {
        if (selectedLocation.slug !== 'any-place') {
          filteredFeaturedPromotions = filteredFeaturedPromotions.filter((promotion) => {
            if (
              promotion.locationsCollection.items.find(
                (element) =>
                  element.slug === selectedLocation.slug || element.slug === 'todo-el-pais',
              ) !== undefined
            ) {
              return true;
            } else {
    ...
    <div ref={sectionRef}>
      ...
      <Layout nav footer banner>
        ...
        <Container className="mx-auto lg:w-10/12 mb-32 lg:mb-72">
          <div className="">
            <div className="my-16 text-3xl font-medium text-center text-blue-600">
              No te pierdas estas súper promociones
            </div>
            ...
            {displayableFeaturedPromotions.length > 0 && (
              <div className="grid my-16 lg:grid-cols-2">
                {displayableFeaturedPromotions.map((promotion) => (
                  <FeaturedPromotionCard key={promotion.slug} {...promotion} />
                ))}
              </div>
            )}
            ...
            <div className="grid my-16 lg:grid-cols-3">
              {displayablePromotions.length == 0 ? (
                <div className="text-center col-span-full">
                  Parece que no se encontraron promociones...
                </div>
              ) : (
                displayablePromotions.map((promotion) => (
                  <PromotionCard key={promotion.slug} {...promotion} />
                ))
              )}
            </div>

            <div data-animation="background-card" className="w-full p-4">
              <Link href="/seguros">
                <a>

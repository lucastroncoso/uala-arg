...
const C01Navigation = ({ content }) => {
  ...
  */
  ...
  const downloadButtonStyle = classNames(styles.customClass, styles.downloadButton, {
    [styles.isVisible]: !isMobile && isScrolled,
  });
  const loginButtonStyle = classNames(styles.loginButton, {
  ...
  */
  ...
  useEffect(() => {
    if (isMobile & isMobileNavOpen) document.body.style.overflow = 'hidden';
        ...
        <div className={itemsWrapperStyle}>
          <ul className={styles.listWrapper}>
            {content.items.map((item, itemIndex) => {
              ...
              const buttonStyle = classNames(styles.itemButton, {
                [styles.isActive]: isCurrentItemActive,
              });
              const iconStyle = classNames(styles.arrowIcon, {
          ...
          <DownloadAppButton
            ...
            qr="/assets/images/qr_ar_menu.svg"
          ...
          {!isMobile && (
            <MSubMenu
              ...
              isWrapped={content.items[selectedItem].subMenu.length > 6}
            />
          )}
        </div>
      </BlockWrapper>
